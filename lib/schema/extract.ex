...
defimpl Schema.Extract, for: List do
...
defimpl Schema.Extract, for: Map do
    def settings(map) do
        Map.get(map, :_vex)
    end
...
defmodule Schema.Extract.Struct do
    @moduledoc false
    defmacro for_struct do
        quote do
            defimpl Schema.Blank, for: __MODULE__ do
                def blank?(struct), do: struct |> Map.from_struct() |> map_size == 0
            end

            defimpl Schema.Extract, for: __MODULE__ do
...
defimpl Schema.Extract, for: Tuple do
