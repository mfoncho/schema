defmodule Schema.Validators.Format do
    @moduledoc """
    ...
    * `:message`: Optional. A custom error message. May be in EEx format
    ...
    ## Examples
        ...
        iex> Schema.Validators.Format.validate("foo", ~r/^f/)
        :ok

        iex> Schema.Validators.Format.validate("foo", ~r/o{3,}/)
        {:error, "must have the correct format"}

        iex> Schema.Validators.Format.validate("foo", [with: ~r/^f/])
        :ok

        iex> Schema.Validators.Format.validate("bar", [with: ~r/^f/, message: "must start with an f"])
        {:error, "must start with an f"}

        iex> Schema.Validators.Format.validate("", [with: ~r/^f/, allow_blank: true])
        :ok

        iex> Schema.Validators.Format.validate(nil, [with: ~r/^f/, allow_nil: true])
        :ok

    ## Custom Error Messages Custom error messages (in EEx format), 
    provided as :message, can use the following values:

        iex> Schema.Validators.Format.__validator__(:message_fields)
        [value: "The bad value", pattern: "The regex that didn't match"]

    An example:

        iex> Schema.Validators.Format.validate("bar", [with: ~r/"^f"/, message: "<%= value %> doesn't start with an f"])
